...
/// Sets libsodium `randombytes` to this implementation and initialises libsodium.  If
/// `optional_seed` is `Some`, then the RNG is seeded with this value, unless `init()` has
...
///
/// Note that this function only ever calls libsodium's `init()` once.  This is reasonable given the
/// current implementation libsodium's `init()`.  However if that should change to make it worth
/// retrying after a failed `init()`, then this function should be updated too.
#[allow(unsafe_code)]
